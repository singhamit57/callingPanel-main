...
if($Postdata != null){
    ...
    $LoginKey = $Postdata['LogedUserLoginKey'];
...
$WorkDuration = 0;
 $getUration = "SELECT TIMESTAMPDIFF(SECOND, LastWorkStamp, '$Stamp') as 'WorkDuration' FROM UserWorkhrs WHERE User_Id = '$LogedUser' AND WorkDate = '$workDate' ";
$result =mysqli_query($conn,$getUration);
if($conn->affected_rows >=1){
    $row = $result->fetch_assoc();
    $WorkDuration = $row['WorkDuration'];
}

if($WorkDuration <=300 and $WorkDuration >=$duration){
    $duration = $WorkDuration;
}

$Update = "UPDATE UserWorkhrs SET WorkDuration = WorkDuration + '$duration', LastWorkStamp = '$workStamp' WHERE Comp_Id = '$CompID' AND User_Id = '$LogedUser' AND WorkDate = '$workDate' ;";
$result =mysqli_query($conn,$Update);
if($conn->affected_rows >=1){

goto Success;
}else{
$querry = "INSERT INTO UserWorkhrs ( Comp_Id, User_Id, WorkDate, WorkDuration, LastWorkStamp ) VALUE ( '$CompID', '$LogedUser', '$workDate',  '$duration', '$workStamp' ); ";
