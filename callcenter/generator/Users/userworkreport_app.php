...
$nowdate = date("Y-m-d");
//$NewDate = date("Y-m-t", strtotime("+ 60 days", strtotime($timestamp))) ;
if($Operation ==""){
    
}else{
    if($Operation =="callreport_today"){
    $FrDate = $nowdate;
    $ToDate = $nowdate ;

}else if($Operation =="callreport_yesterday"){
    $NewDate = date("Y-m-d", strtotime("- 1 days", strtotime($nowdate))) ;
    $FrDate = $NewDate;
    $ToDate = $NewDate ;
    
}else if($Operation =="callreport_last7days"){
    $NewDate = date("Y-m-d", strtotime("- 7 days", strtotime($nowdate))) ;
    $FrDate = $NewDate;
    $ToDate = $nowdate ;
    
}else if($Operation =="callreport_last30days"){
    $NewDate = date("Y-m-d", strtotime("- 30 days", strtotime($nowdate))) ;
    $FrDate = $NewDate;
    $ToDate = $nowdate ;
    
}else if(strpos(strtoupper($Operation), "callreport_")!== false){
...
$DateQue_LeadResponse = " AND ( AddStamp >= '$FrDate 00:00:00' AND AddStamp <= '$ToDate 23:59:59') ";
...
$avgworkhrs = number_format(($workduration/($totalWorkDaysCount*60*60)),2)."";
// $avgworkhrsarr = explode(".",$avgworkhrs);
// if(count($avgworkhrsarr)>=1){
//     // $avgworkhrs = $avgworkhrsarr[0]."";
// }
...
function getWorkduration($seconds,$days){
    
    $day = 0;
    $Hrsarray = explode(".",($seconds/3600));
    $Hrs = $Hrsarray[0];
    $MinSec = ($seconds - 3600*$Hrs);
    $min = explode(".",($MinSec/60));
    $min = $min[0];
    //  if($days ==0){
    //      $format = '%a:%h:%i:%s';
    //  }
...
$DHMworkduration = getWorkduration($workduration,$totalWorkDaysCount) ;
...
$Responseque = "SELECT DISTINCT LastResponse as 'Showresponses' FROM LeadsDataBase WHERE IssuedUser = '$LogedUser' $DateQue_Leadbase ;";
...
$condi = "SELECT COUNT(LeadID) as 'Count' FROM LeadResponse WHERE Userid = '$LogedUser' $DateQue_LeadResponse ;";
