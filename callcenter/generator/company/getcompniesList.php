...
if($conn -> affected_rows >=1){
    while($row =  mysqli_fetch_assoc($result)){
        ...
        //Managers
        $countque = "SELECT COUNT(FullName) as 'Count' FROM UserDetails WHERE CompanyID = '$compId' AND Designation = 'Manager' ;";
        $onedata['UsedManagers'] = getusedcount($countque);
        //Telecallers
        $countque = "SELECT COUNT(FullName) as 'Count' FROM UserDetails WHERE CompanyID = '$compId' AND Designation = 'Telecaller' ;";
        $onedata['UsedTelecallers'] = getusedcount($countque);
        //Admin
        $countque = "SELECT COUNT(FullName) as 'Count' FROM UserDetails WHERE CompanyID = '$compId' AND Designation = 'Admin' ;";
